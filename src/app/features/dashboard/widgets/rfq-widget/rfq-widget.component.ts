...
@Component({
  selector: 'app-rfq-widget',
  standalone: true,
  imports: [CommonModule, DateFormatPipe, BadgeComponent, DetailPopupComponent],
  template: `
    <section class="card" [style.animationDelay.ms]="80">
      <header>
        <h3>RFQ - Requests for Quotation</h3>
      ...
      <table>
        ...
        <tbody>
          <tr *ngFor="let row of visibleRows(); let i = index" [style.animationDelay.ms]="i * 30" (click)="open(row)">
            ...
            <td class="action"><button class="eye" type="button">👁</button></td>
          </tr>
        </tbody>
      </table>
      </div>
    ...
    <app-detail-popup
      *ngIf="selected()"
      [title]="'RFQ Items - ' + selected()?.rfqnumber"
      [headerData]="selected()"
      [items]="items()"
...
export class RfqWidgetComponent implements OnInit {
  ...
  load(): void {
    ...
    this.rfqService.getHeaders(this.auth.getVendorId()).subscribe((rows) => {
  ...
  open(row: any): void {
    ...
    this.rfqService.getItems(String(row.rfqnumber)).subscribe((items) => {
